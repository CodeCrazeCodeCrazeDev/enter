...
class HarnessObserver:
    ...
    async def on_turn_end(self, ctx: TurnContext) -> None:
        # If it's a prompt error (or loop ending unsuccessfully, or general turn end trace)
        # Note: in test_self_harness.py, turn=2 has ai_text="System error context overflow."
        if self.current_trajectory:
            # Let's check if the ai_text has error keywords or is_error triggers
