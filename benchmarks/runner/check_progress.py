...
Supports both single-run (``<run_dir>/trials/<qid>/result.json``) and
multi-run (``<run_dir>/run_<i>/trials/...``) layouts.

Usage:
    uv run python -m benchmarks.runner.check_progress <out_dir>          # snapshot
    uv run python -m benchmarks.runner.check_progress <out_dir> --watch 60
    uv run python -m benchmarks.runner.check_progress <out_dir> --verbose
"""
...
def _scan_trials_dir(trials_root: Path, run_name: str) -> RunStats:
    ...
    for td in sorted(trials_root.iterdir()):
        ...
        if result_path.exists():
            ...
            s.grading[r.get("grading_method", "N/A")] += 1
...
def analyse(run_dir: Path, verbose: bool = False) -> None:
    ...
    accuracy_pct = (n_correct / n_done * 100) if n_done else 0
    ...
    print()
    print("═" * 72)
    print(f"  {_BOLD}AgentHarness Benchmark Monitor{_R}  {_DIM}{now}{_R}")
    ...
    if n_done > 0:
        print(f"  Accuracy    {_bar(accuracy_pct)}   {n_correct}/{n_done}")
    ...
    if is_multi:
        ...
        for s in stats:
            acc = (s.n_correct / s.n_done * 100) if s.n_done else 0
            mark = _colour(acc) if s.n_done else _DIM
            print(f"    {_CYAN}{s.name:<10}{_R}  "
                  f"{mark}{s.n_correct}/{s.n_done}  "
                  f"({acc:5.1f}%){_R}"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Monitor an AgentHarness benchmark run.",
