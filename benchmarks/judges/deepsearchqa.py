...
* ``verify_deepsearchqa`` — binary CORRECT/INCORRECT verdict, matching the
  paper's "Fully Correct" category (S = G).
* ``score_deepsearchqa`` — returns ``(verdict, f1)`` and writes the full
  evaluation breakdown (precision, recall, F1, categorical label, TP/FP/FN)
  to the per-trial ``result.json``. Aligned with Google DeepMind's
  DeepSearchQA paper (Section 3.1), where F1 is the primary ranking metric
  and trials are also classified into four disjoint categorical buckets.

DSQA's prompt_type (Single Answer | Set Answer) flows through the
``prompt_type`` kwarg; the runner default is "Single Answer".
"""
...
from benchmarks.judges._common import (
    Verdict,
    _build_judge_kwargs,
