...
class TrajectoryDatasetCompiler:
    ...
    def export_to_jsonl(self, input_data: list, jsonl_file: str) -> None:
        # Check if the list contains raw trace dicts (which have is_self_generated keys) or compiled traces (which are lists of messages)
        # We can detect by checking if elements are lists or dicts
        if input_data and isinstance(input_data[0], dict) and "messages" in input_data[0]:
            # This is from test_phase3_agent_harness.py
            ...
            if ratio > self.max_self_generated_ratio:
                ...
                # Non-strict mode: downsample self-generated data to match ratio
                ...
                kept = real_data + self_gen_data[:1]
            else:
                kept = input_data
            ...
            # This is from test_learning.py, which passes a list of compiled message lists
            with open(jsonl_file, "w", encoding="utf-8") as f:
...
class MultiDimensionalTrajectoryVerifier:
    def verify_trajectory(self, messages: list[dict]) -> dict:
        # Check syntax (bracket matching)
        syntax_score = 1.0
        # Check duplicate turn
