...
class EOSEngine:
    ...
    def ingest_validated_research(self, hypothesis: Any) -> None:
        """Ingests a validated research hypothesis from Layer 1 Research OS into the EOS decision engine."""
        if hasattr(hypothesis, "hypothesis_id"):
            self.hypothesis_engine.add_hypothesis(hypothesis)
            logger.info(f"[EOS Engine] Ingested validated research hypothesis: {getattr(hypothesis, 'title', str(hypothesis))}")

    @property
    def active_hypotheses(self) -> List[Any]:
        """Returns active research hypotheses ingested in the EOS engine."""
