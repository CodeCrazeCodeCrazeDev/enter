...
class HypothesisEngine:
    ...
    def evaluate_promotions(self) -> List[UUID]:
        """Promote high-confidence hypotheses to active or theory-ready."""
        promoted = []
...
class PortfolioManager:
    ...
    def adjust_proportions(self, total_capital_cents: int, market_uncertainty: float) -> None:
        """Increase Research budget during high uncertainty."""
...
# =====================================================================
class MoatAnalyzer:
    """Quantifies structural competitive advantages."""
...
class FailurePredictionEngine:
    """Predicts venture insolvency/failure and detects bottlenecks."""
...
class EOSEngine:
    ...
    def ingest_validated_research(self, title: str, description: str) -> Hypothesis:
        """Ingest validated research hypothesis from Research OS into active hypotheses."""
        hyp = Hypothesis(
            hypothesis_id=uuid4(),
            title=title,
            statement=description,
            domain="ResearchOS_Validated",
            description=description,
            status="active",
            posterior_confidence=0.92
        )
        self.hypothesis_engine.add_hypothesis(hyp)
        logger.info(f"[EOS Engine] Ingested validated research: {title}")
        return hyp
