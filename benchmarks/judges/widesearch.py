...
Two halves:
1. Structural F1 scorer (rows × items × column-name matching).
2. Async wrapper (``score_widesearch``) that adds a small judge-LLM
   column-alignment pass for label fuzziness.
"""
...
from benchmarks.judges._common import Verdict
...
#   endpoint as benchmarks.judges. Kept sync intentionally — wrapped via
#   asyncio.to_thread() in the async caller.
# ---------------------------------------------------------------------------
_JUDGE_BASE_URL: Optional[str] = None
_JUDGE_API_KEY: Optional[str] = None
...
def _llm_completion(prompt: str, max_retries: int = 3) -> Optional[str]:
    ...
    for attempt in range(max_retries):
        try:
            resp = requests.post(_JUDGE_BASE_URL, headers=headers, json=payload, timeout=120)
...
def primary_key_preprocess(response_list: list, reference_list: list) -> dict:
    ...
    return parse_markdown_json(result) if result else {}
...
def llm_judge_column(response_list: List[str], target_list: List[str], criterion: str) -> tuple:
    response_dict = {f"idx_{i}": {"response": r, "target": t}
                     for i, (r, t) in enumerate(zip(response_list, target_list))}
...
# ---------------------------------------------------------------------------
def extract_dataframe(response_text: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        for col in unique_columns:
            ...
            if "llm_judge" in metrics or "exact_match" in metrics:
                ...
                if pk_map:
                    response_df[col] = response_df[col].apply(lambda x: pk_map.get(x, x))
            ...
            for metric_name in metric_names:
                if metric_name == "llm_judge":
                    ...
                    score_df[f"{col}_{metric_name}"] = df_inner.apply(
                        lambda x, _mf=metric_func, _c=criterion: _mf(
                            x[col + "_response"], x[col + "_query"], _c
                        )[0],
                        axis=1,
                    )
...
def evaluate_from_eval_spec(
    ...
    """Entry point for benchmarks.judges.score_widesearch.
...
async def score_widesearch(
    question: str, ground_truth: str, predicted: str,
    ...
    Args:
        ...
        ground_truth: JSON string of the ``eval_spec`` (with
    ...
    try:
        eval_spec = json.loads(ground_truth) if isinstance(ground_truth, str) else ground_truth
