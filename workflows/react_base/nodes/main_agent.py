...
without calling any tool. When the loop exits without a final answer
(``max_turns`` / ``llm_error`` / budget / etc.),
:func:`_force_final_answer` runs one tool-free LLM call to extract a
plain-text answer.
"""
        ...
        try:
            ...
            if text:
                ...
                logger.info(
                    ...
                    result.stopped_by, attempt + 1, max_retries, len(text),
                )
                _append_traj_event(traj_dir, {
...
async def main_agent_node(
    ...
    if not question:
        raise ValueError(
            "main_agent_node requires 'original_question' in state"
        )
    ...
    tool_result_max_chars = int(agent_cfg.get(
        "tool_result_max_chars", MAIN_TOOL_RESULT_MAX_CHARS,
    ))
    ...
    # Build the summarize prompt up front so the kernel's context-overflow
    # guard can budget for it (the guard estimates summary_prompt tokens
    # as part of the next-call total before deciding to pop history).
    summary_prompt = get_summarize_prompt(question)
    ...
    max_completion_tokens = int(
        profile_llm_cfg.get("max_tokens")
    ...
    logger.info(
        ...
        ctx.task_id, tool_names, max_turns, keep_tool_result,
    ...
    logger.info(
        ...
        ctx.task_id, result.stopped_by, result.turns_used,
    )

    final_text = (
        result.metadata.get("final_answer") or result.final_content
    )
    return {
