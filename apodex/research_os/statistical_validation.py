...
def standard_normal_cdf(x: float) -> float:
    ...
    res = 0.5 * (1.0 + math.erf(x / math.sqrt(2.0)))
    return max(1e-15, min(1.0 - 1e-15, res))
...
def standard_normal_ppf(p: float) -> float:
    ...
    # Clamp probability to strictly safe range [1e-15, 1 - 1e-15]
    p_clamped = max(1e-15, min(1.0 - 1e-15, float(p)))
